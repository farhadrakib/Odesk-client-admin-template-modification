...
@define( "CAPTCHA_IMAGE_SIZE", "350x65" ); // randomly adapted a little
...
#-- operation
@define( "CAPTCHA_TEMP_DIR", easy_captcha_utility::tmp() ); // storage directory for captcha handles
...
define( "CAPTCHA_BASE_URL", siteURL( true, false ) . "includes/functions/captcha.php" );
...
class easy_captcha {
	...
	function is_valid() {
		return isset( $this->id ) && isset( $this->created ) && ( $this->created )
		       && ( $this->expires > time() )
		       && ( $this->tries > 0 )
		       && ( $this->failures < 500 )
	...
	function form( $add_text = "&rarr;&nbsp;" ) {
		...
		$HTML =
			...
			'<img id="captcha_image" src="' . $img_url . '&" width="' . $this->image->width . '" height="' . $this->image->height . '" alt="' . $alt_text . '" align="middle" ' . $onClick . ' title="' . CAPTCHA_REDRAW_TEXT . '" />' .
			'<div class="text">' .
			'<input placeholder="' . $add_text . '" title="' . CAPTCHA_PROMPT_TEXT . '" type="text" ' . $onKeyDown . ' id="' . $p_input . '" name="' . $p_input . '" value="' . ( isset( $_REQUEST[ $p_input ] ) ? htmlentities( $_REQUEST[ $p_input ] ) : "" ) . '" size="8" />' .
			'</div>' .
			$javascript;
	...
	function load() {
		...
		if ( file_exists( $fn ) ) {
			...
			$this->log( "data_file", "SERVER", "captcha file does not exist $fn" );
		}
	}
...
class easy_captcha_text_math_formula extends easy_captcha {
	...
	function solved( $in = null ) {
		return (int) $this->solution == (int) $in;
	}
