...
class DB
{
	...
	function DB($base, $server, $user, $pass)
	{
		...
		mysql_connect($server, $user, $pass) or die('Database connection not possible.');
		mysql_select_db($base) or die('Unable to connect connection not possible.');
	}
	...
	function get_results($query, $debug = -1, $as_obj = true)
	{
		...
		if ($this->lastResult) {
			...
			$dRes = array();
			if ($as_obj)
				while ($row = mysql_fetch_object($this->lastResult))
					$dRes[] = $row;
			else
